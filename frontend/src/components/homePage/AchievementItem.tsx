export default function AchievementItem(prompt:{
  ...
  return (
    <div className="transition ease-in-out delay-20 flex flex-row w-full bg-white
    items-center justify-between rounded-xl relative m-2
    hover:scale-105">
     <div className="flex flex-row items-center m-4">
     <img
          className="w-16 h-16 object-cover mx-auto  mr-4"
          src={prompt.image}
          alt=""
        />
      <div className="flex flex-col my-auto cursor-pointer ">
       
        <p className=" font-bold text-xl text-black">{prompt.title}</p>
  ...
  style={{ borderBottomColor:  achievColor, borderRightColor: achievColor}}
  ></div>
    </div>
  );
}
