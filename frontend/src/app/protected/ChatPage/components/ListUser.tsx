...
<<<<<<< HEAD
    ...
    users.length != 0 ? (
      users.map((el, index) => {
        return (
          <Flex
            align="center"
            className="relative border-b py-2 pl-1"
            key={index}
            style={{
          ...
          >
            <Avatar size="3" src={el.avatar} radius="full" fallback="T" />
  ...
  let styles: string =
    "px-2 py-1 my-2 rounded-[20px] text-[#3055d8] bg-white shadow-md";
  return (
    <Box
      style={{ width: 250, height: 600, borderRadius: 10, background: "white" }}
    >
      ...
      <ScrollArea scrollbars="vertical" style={{ height: 430 }}>
        <Box>
          <Flex direction="column">{userWidget}</Flex>
=======
    const getListUsers = async () => {
      const usersList = await getUserForMsg(user.id);
