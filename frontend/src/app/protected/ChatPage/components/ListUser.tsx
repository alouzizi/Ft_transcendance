...
const ListUser = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      socket?.off("findMsg2UsersResponse", getListUsers);
    };
  }, [socket]);
  ...
  useEffect(() => {
    if (geust.id !== "-1") setDirect(geust.isUser);
  }, [geust.id]);
  ...
  const widgetUser = (el: messageDto, index: number) => {
    return (
      <Flex
        align="center"
        className="relative border-b py-2 pl-3 border-[#E9ECF1] border-1.5"
        key={index}
        style={{
        ...
        {el.isDirectMessage ? (
          <Badge
            ...
            sx={{
              "& .MuiBadge-badge": {
                backgroundColor: `${
                  el.receivedStatus === "ACTIF" && isBlocked === 0
                    ? "#07F102"
                    : "#B4B4B4"
                }`,
                width: 15,
  ...
  return (
    <Box
      className={`bg-white h-[900px] w-[90%] rounded-[15px]
        ${!displayChat ? "" : "hidden"}
        md:block
        md:w-[300px]`}
    >
