...
export default function AlertDialogFind() {
  const { user, setGeust, socket, updateInfo, setOpenAlertError } =
    useGlobalContext();
  ...
  useEffect(() => {
    ...
    const chnls: validChannelDto[] = valideChannels.filter(
      (elm: validChannelDto) => {
  ...
  const widgetItemUser =
    usersFilter.length !== 0 ? (
      usersFilter.map((elm, index) => {
        return (
          <Box p="1" pr="3" key={index} className="mx-5">
  ...
  return (
    <Box>
      ...
      <Dialog open={open} keepMounted onClose={handleClose}>
        <DialogContent className="w-[30rem] h-[20rem]" style={{ padding: 0 }}>
